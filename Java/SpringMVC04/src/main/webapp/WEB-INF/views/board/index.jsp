...
/* 			else {
				listHtml += "<span id='update"+obj.idx+"'><button disabled class='btn btn-success' onclick='goUpdateForm("+obj.idx+")'>수정</button></span> &nbsp&nbsp"
				listHtml += "<button disabled class='btn btn-danger' onclick='goDelete("+obj.idx+")'>삭제</button>"								
			} */
...
/*   		document.querySelector().value
  	...
  	function goUpdate(idx){
  		...
  		$.ajax({
  			url : "board/update",
			type : "put", // put 방식은 JSON 명시를 해줘야함(contentType)
			contentType : "application/json; charset=UTF-8", // 타입명시
			// javascript에서 쓰이는 자바객체(클래스)를 활용해서 보내줘야함 -> JSON 클래스
	...
	<div class="panel-body" style="display:none" id="writeForm">
		<form id="frm">
			...
			<table class="table">
				...
				<tr>
					<td>작성자</td>
					<td><input value="${mvo.memName}" type="text" name="writer" id="writer" class="form-control" readonly></td>
				</tr>
				<tr>
